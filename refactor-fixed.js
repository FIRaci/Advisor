...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  let realEnd = endIdx - 1; // This is the line BEFORE `function CampaignItem({`
  // We need to keep `</div>\n  );\n}`!
  // So let's back up realEnd until we hit `</AnimatePresence>` of GlossaryPanel.
  let currentEnd = realStart;
  while (!lines[currentEnd].includes('</AnimatePresence>')) currentEnd++;
  // Now currentEnd is the AnimatePresence of GlossaryPanel!
...
startIdx = findLine('function CampaignItem({');
if (startIdx !== -1) {
